...
class VideoTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        print("video_test_dataset")
        if opt['name'].lower() in ['vid4', 'myanmar','reds4', 'yuv21','redsofficial','mmeval']:
            if opt['name'].lower() in ['mmeval']:
                for subfolder_lq, subfolder_gt in zip(subfolders_lq, subfolders_gt):
                    # print("subfolders_lq", subfolders_lq)
                    # print("subfolders_gt", subfolders_gt)
                    # get frame list for lq and gt
                    subfolder_name = osp.basename(subfolder_lq)
                    img_paths_lq = sorted(list(scandir(subfolder_lq+ '/' + "input%d" % (self.opt['scale']), full_path=True)))
                    img_paths_gt = sorted(list(scandir(subfolder_gt + '/' + "truth", full_path=True)))
    ...
    def __getitem__(self, index):
        ...
        if self.cache_data:
            ...
            # print("__getitem__ 000000")
        else:
            img_paths_lq = [self.imgs_lq[folder][i] for i in select_idx]
            imgs_lq = read_img_seq(img_paths_lq)
            img_gt = read_img_seq([self.imgs_gt[folder][idx]])
            img_gt.squeeze_(0)
            # print("__getitem__ 111111")
        # print("imgs_lq",imgs_lq.shape)
...
class VideoRecurrentTestDataset(VideoTestDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cache_data:
            ...
            # print("[imgs_lq]",imgs_lq)
            # print("[imgs_gt]",imgs_gt)
        else:
