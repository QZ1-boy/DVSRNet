...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for i in range(rank, num_folders + num_pad, world_size):
            ...
            # visuals['result'] = visuals['result'].unsqueeze(0)
            # visuals['gt'] = visuals['gt'].unsqueeze(1)
            if visuals['result'].dim() == 4:
                visuals['result'] = visuals['result'].unsqueeze(0)
            # print('[visualsresult]',visuals['result'].shape)
            # evaluate
            if i < num_folders:
    ...
    def test(self):
        ...
        # if (h % 32 != 0 ) or (w % 32 != 0 ):
            ...
            # padder = InputPadder_new(self.lq.shape,mode='sintel', padding_factor=32)   #  , mode='sintel'
...
#         # ref, supp = padder.pad(ref, supp)
        ...
        with torch.no_grad():
            ...
            total_time = total_time + time.time()-st_time
            ...
            # if (h % 32 != 0 ) or (w % 32 != 0 ):
                ...
                # print("self.opt['scale']",self.opt['scale'])
                if self.opt['scale'] == 2:
                    self.output = padder.unpadx2(self.output[0])
                if self.opt['scale'] == 3:
                    self.output = padder.unpadx3(self.output[0])
                if self.opt['scale'] == 4:
                    self.output = padder.unpadx4(self.output[0])
                    # self.output = padder.unpadx4(self.output)

                # self.output = padder.unpadx3(self.output)
                # self.output = padder.unpadx4(self.output)
                #print("[self.lq 222]",self.lq.shape)
...
@MODEL_REGISTRY.register()
class VideoRecurrent_flowModel(VideoBaseModel):

    def __init__(self, opt):
        super(VideoRecurrent_flowModel, self).__init__(opt)
        if self.is_train:
            self.fix_flow_iter = opt['train'].get('fix_flow')
    ...
    def optimize_parameters(self, current_iter):
        ...
        super(VideoRecurrent_flowModel, self).optimize_parameters(current_iter)
...
#         # ref, supp = padder.pad(ref, supp)
        ...
        with torch.no_grad():
            ...
            # self.lq = self.lq[:,0:9,:,:,:]
            # self.lq = self.lq.unsqueeze(0)
             #  torch.Size([1, 9, 3, 288, 360])  torch.Size([1, 32, 3, 264, 636])
            st_time=time.time()
            self.output = self.net_g(self.lq)
            total_time = total_time + time.time()-st_time
...
class InputPadder:
    ...
    def unpadx8(self,x):
        ht, wd = x.shape[-2:]
        c = [8*self._pad[2], ht-8*self._pad[3], 8*self._pad[0], wd-8*self._pad[1]]
        return x[..., c[0]:c[1], c[2]:c[3]]



class InputPadder_new:
    """ Pads images such that dimensions are divisible by 8 """
    ...
    def unpadx32(self,x):
        ht, wd = x.shape[-2:]
        c = [32*self._pad[2], ht-32*self._pad[3], 32*self._pad[0], wd-32*self._pad[1]]
        return x[..., c[0]:c[1], c[2]:c[3]]